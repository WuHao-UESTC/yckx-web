...
7. `20260806210000_news_multi_columns`：加入新闻文章与新闻专栏多对多关系，回填现有新闻专栏归属，并清空新闻旧 `columnId`；日常文章的单专栏关系保持不变。
8. `20260806220000_daily_multi_columns`：加入日常文章与日常专栏多对多关系，回填现有日常专栏归属，并清空日常旧 `columnId`。
9. `20260806230000_routine_group_photos`：加入 `PhotoKind`、合照年份和按照片类型查询的复合索引；历史照片默认保持为普通照片墙记录。
10. `20260807090000_profile_contact_fields`：为 `profiles` 增加可空 `grade`、`contactEmail`、`qq`、`wechat` 字段，并为年级增加索引；历史资料无需回填。
11. `20260808100000_external_users_comments_subscriptions`：增加 `GUEST` 角色、邮箱验证与密码重置挑战、用户邮箱验证时间、站内评论状态、订阅、邮件 Outbox，并将历史用户标记为已验证。
...
第七个迁移将普通 `Post(kind = NEWS)` 且原 `columnId` 指向 `Column(type = NEWS)` 的关系复制到 `post_news_columns`，明确排除旧 `EVENT` 大事记文章，随后清空新闻旧 `columnId`。迁移不改写文章、专栏 slug 或正文。回退旧应用前必须恢复迁移前数据库备份；仅删除关联表会使旧应用无法读取已经迁移的新闻专栏归属。

第八个迁移将 `Post(kind = DAILY)` 且原 `columnId` 指向 `Column(type = DAILY)` 的关系复制到 `post_daily_columns`，随后清空日常旧 `columnId`。迁移不改写文章、专栏 slug、照片、留言或正文。回退旧应用前必须恢复迁移前数据库备份；仅删除关联表会使旧应用无法读取已经迁移的日常专栏归属。

第十个迁移只增加可空个人资料字段，不改写用户登录邮箱、文章、头像或上传文件。未填写资料保持 `NULL`；回退应用版本前仍应恢复迁移前数据库备份，避免旧应用读取不存在的 Prisma 字段。

第九个迁移为现有照片增加 `kind` 和可选 `year`。所有历史照片通过数据库默认值归为 `WALL`，不会自动成为顶部合照；只有新应用中的管理员上传流程会创建 `GROUP`。回退旧应用前应恢复迁移前数据库备份，避免旧 Prisma Client 无法识别新增枚举。

第十一个迁移向 `users` 增加可空邮箱验证和密码修改时间，历史账号的 `emailVerifiedAt` 回填为原创建时间，不改变现有登录凭据。新增验证码、订阅、评论状态和邮件 Outbox 表均可由外键级联清理。该迁移不会发送邮件，也不会自动创建订阅；生产应用必须在迁移完成并配置 SMTP 后再允许新注册。
