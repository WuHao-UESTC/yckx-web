...
export default async function AdminPage() {
  const [userCount, postCount, recentUsers, postPublishedCount] = await Promise.all([
    prisma.user.count(),
    prisma.post.count(),
    prisma.user.findMany({ orderBy: { createdAt: "desc" }, take: 5, select: { username: true, displayName: true, createdAt: true } }),
    ...
    <div className="mx-auto max-w-4xl px-5 py-8">
      ...
      <div className="grid gap-4 sm:grid-cols-3 mb-8">
        ...
        <div className="card">
          ...
          <p className="text-sm text-[#6b6b6b] mt-1 font-[family-name:var(--font-sans)]">文章总数（含草稿）</p>
        </div>
      </div>
      ...
      {recentUsers.length > 0 && (
        <div className="mb-8">
          <h2 className="text-sm font-bold text-[#1a1a1a] mb-3 font-[family-name:var(--font-sans)]">最近注册</h2>
          <div className="space-y-1">
            {recentUsers.map((u) => (
              <div key={u.username} className="card flex items-center gap-3 py-2.5 px-4">
                <span className="text-xs text-[#6b6b6b] font-[family-name:var(--font-sans)] w-24">{u.createdAt.toLocaleDateString("zh-CN")}</span>
                <span className="text-sm font-bold text-[#1a1a1a]">{u.displayName ?? u.username}</span>
              </div>
            ))}
          </div>
        </div>
