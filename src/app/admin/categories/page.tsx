...
export default async function AdminCategoriesPage() {
  ...
  async function createCategory(formData: FormData) {
    ...
    await prisma.category.upsert({
      where: { slug },
      update: {},
      create: { name: name.trim(), slug, type: type as "KNOWLEDGE" | "COMPETITION" | "EVENT" },
    });
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-8">
      ...
      <form action={createCategory} className="card mb-8 flex flex-wrap items-end gap-4">
        <div>
          <label className="block text-sm text-[#6b6b6b] mb-1 font-[family-name:var(--font-sans)]">名称</label>
          <input name="name" className="input-field w-40" required placeholder="新分类名" />
        </div>
        <div>
          <label className="block text-sm text-[#6b6b6b] mb-1 font-[family-name:var(--font-sans)]">类型</label>
      ...
      <div className="space-y-2">
        {Object.entries(
          ...
          <div key={type} className="mb-6">
            <h2 className="text-sm font-bold text-[#6b6b6b] uppercase mb-2 font-[family-name:var(--font-sans)]">{typeLabels[type] || type}</h2>
            {cats.map((cat) => (
              <div key={cat.id} className="card flex items-center justify-between mb-1">
                <div>
                  <span className="font-bold text-[#1a1a1a]">{cat.name}</span>
                  <span className="text-xs text-[#6b6b6b] ml-2 font-[family-name:var(--font-sans)]">{cat._count.posts} 篇</span>
                </div>
