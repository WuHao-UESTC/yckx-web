...
export default async function FilesPage() {
  ...
  async function deleteFile(formData: FormData) {
    ...
    const file = await prisma.file.findUnique({ where: { id }, select: { uploaderId: true } });
            ...
            <div key={f.id} className="card flex items-center justify-between flex-wrap gap-2">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <a href={`/api/files/${f.id}`} className="font-bold text-[#1a1a1a] hover:text-[#8b5e3c] text-sm truncate">
                    {f.filename}
                  </a>
                </div>
                <p className="text-xs text-[#6b6b6b] mt-1 font-[family-name:var(--font-sans)]">
                  {(f.size / 1024).toFixed(0)} KB · {f.mimeType} · {f.createdAt.toLocaleDateString("zh-CN")}
                  {f.post && <span className="ml-2">📎 {f.post.title}</span>}
                </p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <a href={`/api/files/${f.id}`} className="text-xs text-[#8b5e3c] hover:text-[#5a3a22] font-[family-name:var(--font-sans)]">下载</a>
