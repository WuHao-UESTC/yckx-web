  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-8">
      ...
      <div className="card mb-8 text-center">
        ...
        {user.profile?.title && (
          <p className="text-[#8b5e3c] text-sm mt-1 font-[family-name:var(--font-sans)]">{user.profile.title}</p>
        )}
        {user.bio && (
          <p className="text-[#6b6b6b] text-sm mt-2 max-w-md mx-auto font-[family-name:var(--font-sans)]">{user.bio}</p>
        )}
        <div className="flex justify-center gap-4 mt-3 text-xs text-[#6b6b6b] font-[family-name:var(--font-sans)]">
          ...
          <span>·</span>
          <span>{Math.floor((Date.now() - user.createdAt.getTime()) / 86400000)} 天前加入</span>
        </div>
        {user.profile && (
          <div className="flex justify-center gap-3 mt-3">
            {user.profile.website && <a href={user.profile.website} target="_blank" rel="noopener noreferrer" className="text-sm text-[#8b5e3c]">🌐 网站</a>}
            {user.profile.github && <a href={`https://github.com/${user.profile.github}`} target="_blank" rel="noopener noreferrer" className="text-sm text-[#8b5e3c]">💻 GitHub</a>}
            {user.profile.bilibili && <a href={`https://space.bilibili.com/${user.profile.bilibili}`} target="_blank" rel="noopener noreferrer" className="text-sm text-[#8b5e3c]">📺 B站</a>}
          </div>
        )}
      </div>

      {/* 文章列表 */}
      <h2 className="text-xl font-bold text-[#1a1a1a] mb-4 pb-2 border-b border-[#e8e0d5]">发表的文章</h2>
      {posts.length === 0 ? (
