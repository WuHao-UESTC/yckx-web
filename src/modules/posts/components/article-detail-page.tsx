    ...
    >
      ...
      <div className="article-reading-frame">
        <article className="article-reading-sheet">
          ...
          {post.newsColumns.length > 0 && (
            <nav className="article-technical-columns" aria-label="所属新闻专栏">
              <span>所属专栏</span>
              {post.newsColumns.map(({ column }) => (
                <a key={column.id} href={`/archive/columns/${column.slug}`}>
                  {column.title}
                </a>
              ))}
            </nav>
